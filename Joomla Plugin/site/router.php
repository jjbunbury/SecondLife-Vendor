...
function SlvendorBuildRoute(&$query)
{
	static $items;
	...
	// Break up the product/category id into numeric and alias values.
	...
	if (is_array($items))
	{
		...
		if (!$itemid)
		{
			foreach ($items as $item)
			{
				if (isset($query['view']) && $query['view'] == 'product'
					&& isset($item->query['view']) && $item->query['view'] == 'category'
					&& isset($item->query['id']) && isset($query['catid'])
					&& $query['catid'] == $item->query['id'])
				{
					// This menu item links to the product view but we need to append the product id to it.
					$itemid		= $item->id;
		...
		if (!$itemid)
		{
			foreach ($items as $item)
			{
				if (isset($query['view']) && $query['view'] == 'product' && isset($item->query['view'])
					&& $item->query['view'] == 'categories' && isset($query['catid']) && isset($query['id']))
				{
					// This menu item links to the categories view but we need to append the category and product id to it.
					$itemid		= $item->id;
					$segments[]	= isset($query['catalias']) ? $query['catid'].':'.$query['catalias'] : $query['catid'];
					$segments[]	= isset($query['alias']) ? $query['id'].':'.$query['alias'] : $query['id'];
					break;
...
function SlvendorParseRoute($segments)
{
	$vars	= array();
	...
	if (is_object($item))
	{
		...
		if (isset($item->query['view']) && $item->query['view'] == 'product' && isset($segments[0]))
		{
			// Contact view.
			$vars['view']	= 'product';
			$vars['id']		= $segments[0];
